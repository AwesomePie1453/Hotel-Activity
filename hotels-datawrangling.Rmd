...
The most is 2 adults. 403 with no adults is suprirsing- kids rule the world? Also 26 and 27 is there but no 21-25? And missing many of the teens. Weird
...
hotels %>%
  count(adults, is_canceled == 0)%>%
  arrange(desc(n))
```
Most of the children parties canceled! 
...
hotels%>%
  group_by(hotel)%>%
  summarize(min_adr = min(adr), mean_adr=mean(adr), median_adr=median(adr), max_adr = max(adr))
```
